...
Guidelines:
- "last 4 digits visible" / "show last 4" / "****1234" → show_last_n_digits, n=4
- "mask last 4 digits" / "hide last digit" → mask_last_n_digits, n=<N or 1>
- "show first 6 digits" → show_first_n_digits, n=6
- "keep first 3 characters" → show_first_n_chars, n=3
...
- "mask everything" / "full mask" / "hide all" → mask_all
- "keep format" / "format-preserving" → format_preserve_mask
- If n is not stated, default to 4 for digit ops, 3 for char ops.
...
def _keyword_normalize(rule: str) -> Optional[Dict[str, Any]]:
    ...
    if re.search(r"\blast\s+digit\b", t):
        return {"type": "show_last_n_digits", "n": 1}

    # "show/keep first N digit(s)"
    m = re.search(
    ...
    # format-preserving — keep separators, replace digit positions with *
    if re.search(
...
def normalize_masking_rule(
    ...
    Example return values:
      {"type": "show_last_n_digits", "n": 4}
      {"type": "mask_last_n_digits", "n": 2}
      {"type": "redact"}
      {"type": "partial_email"}
    """
        ...
        try:
            ...
            if not isinstance(llm_provider, DemoProvider):
                ...
                if isinstance(op, dict) and op.get("type") in OP_TYPES:
                    # Coerce n/size to int if present
                    for k in ("n", "size"):
                        if k in op:
                            op[k] = int(op[k])
                    return op
...
def apply_masking_op(value: Any, op: Dict[str, Any]) -> str:
    ...
    if op_type == "partial_email":
        if "@" in sv:
            ...
            if len(local) <= 1:
                return sv  # too short to mask
            return local[0] + "*" * (len(local) - 1) + "@" + domain
        # Not an email — fall through to mask_all
        return re.sub(r"[A-Za-z0-9]", "*", sv)

    # ── date_year_only ────────────────────────────────────────────────────────
    if op_type == "date_year_only":
        # ISO dates: 1990-01-15 → 1990, or 01/15/1990 → 1990
        m = re.search(r"\b((?:19|20)\d{2})\b", sv)
        if m:
            return m.group(1)
        return sv  # can't parse, return as-is
    ...
    if op_type == "format_preserve_mask":
        # Replace digit chars with *, keep all other characters (dashes, spaces, etc.)
